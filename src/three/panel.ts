...
export function createPanel(ctx: PanelCtx): void {
  ...
  addRow({ label: 'Ribbon separation', min: 0, max: 6, step: 0.1, fmt: (v) => v.toFixed(1), hint: 'Extra gap (car-widths) kept between DISTINCT ribbons, on top of the road width. Stops direct adjacency/overlap; drive freely, only running alongside existing road is refused. Applies live.', get: () => rc.laneGapCars, set: (v) => (rc.laneGapCars = v) });
  addRow({ label: 'Crossing angle', min: 10, max: 80, step: 1, fmt: int, hint: 'Meet existing road steeper than this = a crossing (clean intersection, allowed); shallower = an overlap (refused). Lower = more crossings allowed.', get: () => rc.crossAngleDeg, set: (v) => (rc.crossAngleDeg = v) });
