...
export function createPanel(ctx: PanelCtx): void {
  ...
  addRow({ label: 'Emergency guard', min: 0, max: 5, step: 0.1, fmt: (v) => `${v.toFixed(1)} cars`, hint: 'In an emergency the arms never eat rail within this many car lengths of you (under you and just behind).', get: () => rc.cannibalGuard, set: (v) => (rc.cannibalGuard = v) });
  addRow({ label: 'Emergency guard ahead', min: 0, max: 20, step: 0.5, fmt: (v) => `${v.toFixed(1)} cars`, hint: 'Nor any rail straight ahead of you out to this many car lengths: the road you are about to need. Everything else is fair game.', get: () => rc.cannibalGuardAhead, set: (v) => (rc.cannibalGuardAhead = v) });
  ...
  addRow({ label: 'Eraser reach', min: 0, max: 1500, step: 10, fmt: (v) => (v <= 0 ? 'off' : int(v)), hint: 'Launch while OFF the rail, facing road, and the drone erases the road straight ahead of you (up to this far) instead of peeling an end: for clearing a malformed bit so you can lay it clean. It can cut your network. Riding the rail, launch is always the end cleanup. 0 = eraser off.', get: () => rc.eraserReach, set: (v) => (rc.eraserReach = v) });
  addRow({ label: 'Eraser size', min: 10, max: 400, step: 5, fmt: int, hint: 'Radius of the patch the eraser lifts around the road it hits.', get: () => rc.eraserRadius, set: (v) => (rc.eraserRadius = v) });
