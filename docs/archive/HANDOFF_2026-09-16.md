# Moon Miner — Handoff (archived 2026-09-24)

> **Archived.** This is the handoff as it stood on 2026-09-16, just after the Three.js flip, with the 2026-09-23 note. §0 was carried into the current `HANDOFF.md` at the repo root. §1–§8 describe the Phaser build (URL flags, `src/scenes/*`, the debug snapshot, the 74/9 test state) and are kept for intent and rationale only. Archived under ECO-28A (quarantine, not deletion) by DEV-58.
